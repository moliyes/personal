...
clc
clear
data=dlmread('MECHANICS_3D.dat','',2,0);
phi=data(:,1)*pi/180;
theta=data(:,2)*pi/180;

disp(" ======================= Plot Options ===========================")
disp(" 1) Bulk Modulus")
disp(" 2) Young's Modulus")
disp(" 3) Shear Modulus (minimum)")
disp(" 4) Shear Modulus (maximum)")
disp(" 5) Poisson's ratio (minimum)")
disp(" 6) Poisson's ratio (maximum)")
disp(" 7) Linear Compressibility")
n = input(' Which mechanical quantity do you want to plot? ');

R=data(:,n+2);   % 3 for Bulk; 4 for Young; 5 for Shear_min; 6 for Shear_max; 7 for Poisson_min; 8 for Poisson_max; 9 for Compressibility.
nphi=500;
ntheta=500;
...
colormap jet
surf(x,y,z,Rnew,'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud')
...
flex_factor=1.3;
mArrow3([0 0 0],[flex_factor*max(R) 0 0],'color','red','stemWidth',max(R)/150,'facealpha',0.5);
mArrow3([0 0 0],[0 flex_factor*max(R) 0],'color','green','stemWidth',max(R)/150);
mArrow3([0 0 0],[0 0 flex_factor*max(R)],'color','blue','stemWidth',max(R)/150);
